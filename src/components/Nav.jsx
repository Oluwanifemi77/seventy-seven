...
export default function Nav({ cartCount, menuOpen, onMenuToggle }) {
  ...
  const handleCartClick = () => {
    if (cartCount === 0 && badgeRef.current) {
      const btn = badgeRef.current.closest('button')
      btn?.animate(
        [
  ...
  return (
    <nav
      className={`nav${scrolled ? ' scrolled' : ''}${menuOpen ? ' menu-open-nav' : ''}`}
      id="nav"
      aria-label="Primary"
    >
      <a href="#top" className="nav__brand" aria-label="77 Seventy Seven — home" onClick={(e) => handleLink(e, '#top')}>
        <span className="logo">
      ...
      <div className="nav__links" role="navigation">
        {[['#top', 'Home'], ['#shop', 'Shop'], ['#lookbook', 'Lookbook'], ['#about', 'About'], ['#contact', 'Contact']].map(([href, label]) => (
          <a key={href} href={href} onClick={(e) => handleLink(e, href)}>{label}</a>
        ))}
      </div>

      <div className="nav__right">
        <button className="nav__cart" aria-label="Cart" onClick={handleCartClick}>
